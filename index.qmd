...
# Configure reticulate to use misy331_base conda environment
if (!requireNamespace("reticulate", quietly = TRUE)) {
  install.packages("reticulate")
}
library(reticulate)

# Set the Python path explicitly before any other operations
python_path <- "/opt/anaconda3/envs/misy331_base/bin/python"

# Verify the Python executable exists
if (!file.exists(python_path)) {
  stop("Python not found at: ", python_path, "\nPlease check your conda environment setup.")
}

# Set environment variable for reticulate
Sys.setenv(RETICULATE_PYTHON = python_path)

# Try multiple methods to set Python
# Method 1: Use conda environment by name
tryCatch({
  use_condaenv("misy331_base", required = TRUE)
  cat("✓ Successfully activated conda environment: misy331_base\n")
}, error = function(e) {
  cat("Method 1 failed, trying direct path...\n")
  # Method 2: Use Python path directly
  tryCatch({
    use_python(python_path, required = TRUE)
    cat("✓ Using Python at:", python_path, "\n")
  }, error = function(e2) {
    # Method 3: Force set the Python
    py_config_set(python = python_path)
    cat("✓ Forced Python path to:", python_path, "\n")
  })
})
...
if (py_available()) {
  ...
  cat("  Python executable:", config$python, "\n")
} else {
  stop("✗ Python is still not available after configuration.\n",
       "  Tried path: ", python_path, "\n",
       "  Please check your conda environment setup.")
}
```
...
# Choose layout: 1×4 for wide images, 2×2 for square/tall images
if aspect_ratio > 1.5:
    # Wide image: use 1×4 horizontal layout
    fig, axes = plt.subplots(1, 4, figsize=(16, 4))
    layout = 'horizontal'
else:
    # Square or tall image: use 2×2 grid layout
